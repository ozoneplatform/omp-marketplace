...
class ScheduledImportServiceUnitTest extends Specification implements DataTest, ServiceUnitTest<ScheduledImportService>{
    ...
    private mockImportMethods(Collection<String> except) {
        ...
        if (!except.contains('profiles'))
            service.metaClass.importProfiles = doNothing
        if (!except.contains('categories'))
            service.metaClass.importCategories = doNothing
        if (!except.contains('types'))
            ScheduledImportService.metaClass.importTypes = { Collection c, ImportStatus s -> }
        if (!except.contains('states'))
            service.metaClass.importStates = doNothing
    ...
    private void mockRestServices() {
        [
            ...
            if(cls == 'CheckBoxCustomFieldDefinitionRestService')
                service.checkBoxCustomFieldDefinitionRestService.metaClass.createdDate = []
            service[property] = Mock(cls) {
                createFromDto(*_) >> {dto, skipValidation ->
                    dto.id = 10

                    //if(grailsApplication.isArtefactOfType('DOMAIN', clsName)) {
                        createdDate << dto
                ...
                updateById(*_) >> { id, dto, skipValidation ->
                    assert id == dto.id
      ...
      //      cls.metaClass.createdDate = []
        //    cls.metaClass.updatedDate = []
//            service[property].getClass().metaClass.created = []
//            service[property].getClass().metaClass.updated = []
        }
    }
    ...
    private mockListMethods() {
        ...
        CheckBoxCustomFieldDefinition.metaClass.'static'.list = { customFieldDefinitionList }
    ...
    void testExecuteScheduledImportWithId() {
        ...
        then:
        task.id == taskPassedIn.id
        task.name == taskPassedIn.name
    }
    ...
    void testImportProfiles() {
        ...
        service.metaClass.importTypes = { Collection c, ImportStatus s -> }
        service.executeScheduledImport(task)
        ...
        result.result == true
        profiles.size() == 2
        usernames.contains('testAdmin1')
        usernames.contains('testUser1')
        !usernames.contains('System')    //System user does not come through in import
        ...
        profiles[0].username == importData.profiles[2].username
    }
