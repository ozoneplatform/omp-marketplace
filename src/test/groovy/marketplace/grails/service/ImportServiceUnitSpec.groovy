...
class ImportServiceUnitSpec extends Specification implements ServiceUnitTest<ImportService>, DataTest{
	...
	void testMappingNoAgency() {
		...
		updatedJson =  importService.updateJSON(importFile)
		then:assert(updatedJson.serviceItems[0].agency==null)
	}
	...
	void testGetRequiredListings() {
		...
		expect:
		assert 2 == si0Requires.size()
		assert 1 == si0Requires[0]
		assert 2 == si0Requires[1]
		assert 1 == si1Requires.size()
		assert 2 == si1Requires[0]
		assert 0 == si2Requires.size()
	}
