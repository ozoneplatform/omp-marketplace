...
trait DomainConstraintsUnitTest<T extends GroovyObject & GormValidateable>
    ...
    void urlPropertyHasMaxSize(String propertyName, int maxSize) {
        propertyValueIsValid(propertyName, "http://" + getStringOfLength(maxSize - 8))
        propertyValueIsValid(propertyName, "http://" + getStringOfLength(maxSize - 7))
        propertyValueIsTooLarge(propertyName, "http://" + getStringOfLength(maxSize - 6))
    }
    ...
    private static String getStringOfLength(Long number) {
        'A' * number
    }
