...
class ServiceItemActivityInternalServiceSpec extends Specification implements DataTest, ServiceUnitTest<ServiceItemActivityInternalService> {
//    ServiceItemActivityInternalService service
    ...
    void setup() {
        ...
        service.profileRestService = Mock(ProfileRestService) {
            getCurrentUserProfile(*_) >> {return currentUser }
        }

        service.marketplaceMessagingService = Mock(MarketplaceMessagingService) {
            sendNotificationOfChange(*_) { serviceItem, activity -> []}
        }
//        marketplaceMessagingServiceMock.demand.sendNotificationOfChange(999) { serviecItem, activity -> [] }
    }
    //TODO BVEST Review
    @Ignore
    void testAddServiceItemActivityByAction() {
        when:
        ...
        then:
        si.serviceItemActivities.size() == 1
        si.serviceItemActivities[0].action == Constants.Action.DISABLED
        si.serviceItemActivities[0].author == currentUser
        si.lastActivity == si.serviceItemActivities[0]
    }
