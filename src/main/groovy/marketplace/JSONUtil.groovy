...
class JSONUtil {
  ...
  */
    ...
    static def addCreatedAndEditedInfo(JSONObject jsonObject, AuditStamped domainObj) {
        jsonObject.put("createdDate", domainObj.createdDate ?: new Date())
        if (domainObj.createdBy) {
            Profile createdBy = domainObj.findCreatedByProfile()
            jsonObject.put("createdBy", new JSONObject(id: createdBy?.id, name: createdBy?.display(), username: createdBy?.username))
        }

        jsonObject.put("editedDate", domainObj.editedDate ?: new Date())
        if (domainObj.editedBy) {
            Profile editedBy = domainObj.findEditedByProfile()
            jsonObject.put("editedBy", new JSONObject(id: editedBy?.id, name: editedBy?.display(), username: editedBy?.username))
        }
    }
