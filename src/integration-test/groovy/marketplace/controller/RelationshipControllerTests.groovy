...
@Integration
@Rollback
class RelationshipControllerTests extends MarketplaceIntegrationTestCase implements ControllerTestMixin<RelationshipController> {
...
//		super.setUp()
    ...
    */
	...
	void testGetRelatedItems() {
		...
		then:
        true ==  controllerResponse.success
		3 == controllerResponse.totalCount
		"Requires 1" == controllerResponse.data[0].title
		"Requires 2" == controllerResponse.data[1].title
		"Requires 3" == controllerResponse.data[2].title
		null == controllerResponse.data[2].versionName
//        JSONObject.NULL == controllerResponse.data[2].versionName
	}
	...
	void testGetRelatedItemsErrorCase() {
		...
		def controllerResponse = JSON.parse(controller.response.contentAsString)
		then:
        false == controllerResponse.success
	}
	...
	void testGetOWFRequiredItems() {
		...
		def controllerResponse = JSON.parse(controller.response.contentAsString)
		then:
        4 == controllerResponse.total
        "Parent" == controllerResponse.data[0].title
        3 == controllerResponse.data[0].requires.size()

        when:
        def expectedIds = [requires1.id, requires2.id, requires3.id] as Set
        logIt "${controllerResponse.data[0].requires}"
        then:
        expectedIds == controllerResponse.data[0].requires.collect {it.id as Long } as Set

		"Requires 1" == controllerResponse.data[1].title
		"Requires 2" == controllerResponse.data[2].title
	}
    ...
    void compareRequiresItem(def expectedItem, def resultsItem){
      ...
      //  expect:
        expectedItem.id == resultsItem.id
        expectedItem.title == resultsItem.title
        expectedItem.uuid == resultsItem.uuid
    }
