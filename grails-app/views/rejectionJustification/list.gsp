    ...
    <body>
        <div id="marketContentWrapper" class="widget-marketContentWrapper-wleftbar bootstrap-active">
          <div id="marketContent" style="padding-top: 2%; ">
		    ...
		    <div class="body">
		      <h1><g:message code="rejectionJustification.list.title" encodeAs="HTML" /></h1>
		      ...
		      <div class="list">
		        <table class="listTables table" cellpadding="0" cellspacing="0">
		          ...
		          <tbody>
		          <g:each in="${rejectionJustificationList}" status="i" var="rejectionJustification">
		            <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
		              <td class="admin_page_list"><g:link action="show" id="${rejectionJustification.id}">${fieldValue(bean:rejectionJustification, field:'title')}</g:link></td>
		              <td class="admin_page_list">${fieldValue(bean:rejectionJustification, field:'description').replaceAll("\n","<br>")}</td>
		            </tr>
		          </g:each>
		          </tbody>
		        </table>
		      ...
		      <div class="admin_paginate_buttons">
		        <g:paginate total="${rejectionJustificationTotal}" />
		      </div>
		      <span class="menuButton admin_page_link"><g:link class="admin_create" action="create"><g:message code="rejectionJustification.create.title" encodeAs="HTML" /></g:link></span>
		    </div>
		   </div>
		 </div>
  </body>
