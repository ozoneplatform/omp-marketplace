...
class ElasticSearchService implements GrailsApplicationAware {
    ...
    private SearchRequest buildSearchRequest(query, filter, Map params) {
        ...
        if (query) {
            switch(query) {
                case String:
                    setQueryInSource(source, (String) query, params)
                    break
                case Closure:
                    setQueryInSource(source, (Closure) query, params)
                    break
                case QueryBuilder:
                    setQueryInSource(source, (QueryBuilder) query, params)
                    break
                default:
                    log.error("Unsupported query type: ${query.class.canonicalName}")
            }
        }
        ...
        if (filter) {
            switch(filter) {
                case SearchSourceBuilder:
                    setFilterInSource(source, ((SearchSourceBuilder) filter).postFilter())
                    break
                case Closure:
                    setFilterInSource(source, (Closure) filter)
                    break
                case QueryBuilder:
                    setFilterInSource(source, (QueryBuilder) filter)
                    break
                default:
                    log.error("Unsupporter filter type: ${filter.class.canonicalName}")
            }
        }
    ...
    SearchSourceBuilder setFilterInSource(SearchSourceBuilder source, Closure filter){
    ...
    SearchSourceBuilder setFilterInSource(SearchSourceBuilder source, QueryBuilder filter){
