	...
	_send: function(dfd, method, authTarget, onAuthLoad){
		...
		if(dojo.isMoz && ![].reduce){
			...
			frameContainer = doc.body;
                ...
                var frame = ioArgs.frame = frame = doc.createElement(dojo.isIE ? '<iframe src="' + dojo.moduleUrl("dojox", "io/init_src.html") + '" name="' + frameName + '" onload="dojox.io.windowName['+frameNum+']()">' : 'iframe');
		styleFrame(frame);
